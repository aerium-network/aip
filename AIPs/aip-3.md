...
description: Blockchain-native intellectual property system providing tamper-proof ownership records, version control, and transfer capabilities for digital assets.
...
- **Transfer Payload** - Native token transfers between addresses
- **Bond Payload** - Validator staking operations
- **Unbond Payload** - Validator unstaking operations
- **Withdraw Payload** - Reward withdrawal operations
- **Sortition Payload** - Consensus participation mechanism
- **Batch Transfer Payload** - Multiple transfers in single transaction

### New IP Registry Payload

This PIP proposes the addition of a new payload type: **IP Registry** (PayloadType: `7`).

The IP Registry payload supports three distinct operations differentiated by field presence and validation rules:

1. **REGISTER** - Initial intellectual property registration
2. **UPDATE** - Content or metadata modification while preserving ownership
3. **TRANSFER** - Ownership transfer to a new address

| Operation | Fixed Fields | Optional Metadata | Total Range |
|-----------|-------------|-------------------|-------------|
| **REGISTER** | 153 bytes | 0-372 bytes | 153-525 bytes |
| **UPDATE** | 185 bytes | 0-372 bytes | 185-557 bytes |
| **TRANSFER** | 218 bytes | 0-372 bytes | 218-590 bytes |

## Payload Structure

### Common Fields

All IP Registry operations share the following base structure:
...
| Version | `uint32` | 4 Bytes | Sequential version number (0, 1, 2, 3, ...) | ✅ |
| Signature | `Bytes` | 64 Bytes | Ed25519/ECDSA signature of computed ProofHash | ✅ |

### Optional Metadata Fields

All operations support optional metadata for selective privacy control:
...
**Additional Fields**: None (uses only common fields)

**Validation Rules**:
- `Version` MUST equal `0`
- `ContentHash` MUST be unique (not previously registered)
- `PreviousHash` field MUST be absent
- `NewOwnerPublicKey` field MUST be absent

**ProofHash Calculation**:

Where:
- `||` represents byte array concatenation
- `VersionBytes = BigEndian.Uint32ToBytes(Version)`
...
|-------|------|------|-------------|----------|
| PreviousHash | `Hash` | 32 Bytes | Hash of previous IP transaction | ✅ |
| NewOwnerPublicKey | `Bytes` | 32 Bytes | Compressed public key of new owner | ✅ |

**Validation Rules**:
- `Version` MUST equal `CurrentState.Version + 1`
- `Owner` MUST equal current owner in state
- `PreviousHash` MUST equal `CurrentState.LastTransactionHash`
- `NewOwnerPublicKey` MUST be valid compressed public key

**ProofHash Calculation**:

$$
ProofHash_{TRANSFER} = H(ContentHash \parallel MetadataHash \parallel Version \parallel PreviousHash \parallel NewOwnerPublicKey)
$$
...
- **Quantum Resistance**: Post-quantum secure against known quantum algorithms (Grover's algorithm reduces security to `2^128`, still adequate)
